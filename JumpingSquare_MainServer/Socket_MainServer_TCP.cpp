...
int main()
{
	...
	serverAddr.sin_port = htons(UDP_PORT);			// 연결할 Port #
	serverAddr.sin_addr.s_addr = inet_addr(IP_ADDRESS); // 타켓 IP
	...
	while (true)
	{
		...
		{
			DWORD dwThreadID;
			HANDLE hThread = CreateThread(NULL, 0, MessageThreadTCP, (LPVOID)ClientSocket, 0, &dwThreadID);
		}
		else
		{
...
DWORD WINAPI MessageThreadTCP(LPVOID pvParam)
{
	DWORD dwResult = 0;
	char buf[BUF_SIZE];
